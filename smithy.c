...
void upgrade_menu(HANDLE con, entity_t *player, inventory_t *inv, int choice){
    ...
    sprintf(buffer, "1.Upgrade armour (DEF: %d -> %d) for %d ores and %d gold coins", player->armour, (int)(player->armour * 1.2 + 2), arm_p_o, arm_p_g);
    sprintf(buffer1, "2.Upgrade weapon (BASE ATK: %d -> %d) for %d ores, %d gold coins", player->dmg, (int)(player->dmg * 1.2 + 3), weap_p_o, weap_p_g);
    ...
    if(choice == 0){
        if(player->inv->items[6].quantity < arm_p_o || player->coins < arm_p_g){
            ...
            if(choice == 0)
                upgrade_menu(con, player, inv, choice);
            else
                return;    
        }
        else{
            upgrade_armour(con, player, inv, choice);
        }
    }
    else if(choice == 1){
        if(player->inv->items[6].quantity < weap_p_o || player->coins < arm_p_g){
            ...
            upgrade_weapon(con, player, inv, choice);
        }
    }
}
...
void upgrade_armour(HANDLE con, entity_t *player, inventory_t *inv, int choice){
    ...
    player->armour_lvl++;
    player->armour += (0.5 * player->armour);
...
void smithy(HANDLE con, entity_t *player, inventory_t *inv, int choice){
    ...
    choice = display_dialogue_box(con, "Welcome to my smithy, adventurer, I hope I can help you.\n", (char *[]) {"Continue...", "Leave"}, 2);
    if(choice == 0){
        upgrade_menu(con, player, inv, choice);
    }
    else{
